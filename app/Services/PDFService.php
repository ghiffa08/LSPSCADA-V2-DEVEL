...
class PDFService
{
     ...
     * @param object|null $headerData Optional header data from HeaderService.
     * @return void
     */
    public function generateMultiPagePdf(array $views, array $data, string $filename, ?object $headerData = null): void
    {
        try {
            ...
            $pdf->SetAuthor('LSP-P1 SMKN 2 Kuningan');
            $pdf->SetTitle($filename);

            if ($headerData && !empty($headerData->logo)) {
                $pdf->SetHeaderData($headerData->logo, $headerData->logo_width, $headerData->title, $headerData->header_string);
            } else {
                // Fallback ke konfigurasi default dari tcpdf_config.php
