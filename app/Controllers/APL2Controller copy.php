...
class APL2Controller extends BaseController
{
    ...
    {
        $dataAPL1 = $this->apl1Model->getAPL1Name($name);
        $data = [
            'siteTitle' => "Asesmen Mandiri",
            'dataAPL1' => $dataAPL1,
            'listUnit' => $this->unitModel->getUnit($dataAPL1['id_skema']),
    ...
    {
        $dataAPL1 = $this->apl2Model->getAllAsesmen($id_apl1);
        $listUnit = $this->unitModel->getUnit($dataAPL1['skema_id']);
        $listElemen = $this->elemenModel->AllElemen();
        $listAsesi = $this->apl2Model->getbySkema($dataAPL1['skema_id']);
    ...
    public function validasi()
    {
        $data = [
            ...
            'listAPL2Pending' => $this->apl2Model->getPendingData2(),
        ];
        // dd($data);
    ...
    public function store_validasi()
    {
        ...
        $this->apl2Model->update($this->request->getVar('id'), $data);
    ...
    public function send_email_validasi()
    {
        ...
        $listAPL2Validated = $this->apl2Model->getEmailValidasiToday();
        ...
        foreach ($listAPL2Validated as $row) {
            ...
            if ($row['validasi_apl2'] == "validated") {
                ...
                $message = view('email/email_validated_apl2', [
                    'dataAPL1' => $this->apl2Model->emailDetailSertifikasi($row['id_apl1']),
                ]);
            } else {
            ...
            if ($email->send()) {
                ...
                if ($this->apl2Model->update($row['id_apl2'], $data)) {
    ...
    public function send_email_validasi_by_date()
    {
        ...
        $listAPL2Validated = $this->apl2Model->getEmailValidasiByDate($this->request->getVar('dateValidated'));
        ...
        foreach ($listAPL2Validated as $row) {
            ...
            if ($email->send()) {
                ...
                $this->apl2Model->update($row['id_apl2'], $data);
            } else {
                $data = $email->printDebugger(['headers']);
                print_r($data);
            }
    ...
    {
        $dataAPL2 = $this->apl2Model->getEmailValidasiByDate($this->request->getVar('dateValidated'));
        $no = null;
        $output = '';
    ...
    public function scan_ttd_asesor($ttd)
    {
        return view('scan/scan-ttd-asesor', [
            'data' => $this->apl2Model->getbyttdAsesor($ttd),
...
//     // HTML message
//     $message = view('email/email_validated_apl2', [
//         'dataAPL1' => $this->apl2Model->emailDetailSertifikasi($this->request->getVar('id_apl1')),
//     ]);
// } else {
//     $message = view('email/email_unvalidated_apl2', [
...
// }
    ...
    //     $dataAPL1 = $this->apl1Model->getAPL1byid($this->request->getVar('id_apl1'));
    ...
    //     $validated = $this->apl2Model->update($this->request->getVar('id'), $data);
