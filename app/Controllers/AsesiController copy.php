    ...
    public function save()
    {
        ...
        try {
            ...
            if (!$this->validate($userRules)) {
                throw new \Exception('Validasi data user gagal: ' . json_encode($this->validator->getErrors()));
            }
            ...
            // Process signature from canvas (base64)
            if (!empty($signatureData) && strpos($signatureData, 'data:image') === 0) {
                ...
                // Process signature from file upload
            ...
            // If we have a new signature, handle old one and update data
            if ($newSignatureName) {
            ...
            // 3. Handle asesi data if needed
            if ($this->request->getVar('update_asesi') || !$isUpdate) {
                // For new entries, generate a unique ID
                if (!$isUpdate) {
                    $id = generate_application_id('ASI', 'asesi', 'id_asesi');
                }

                // Validate asesi form data
                if (!$this->validateFormData($isUpdate)) {
                    throw new \Exception('Validasi data asesi gagal: ' . json_encode($this->validator->getErrors()));
                }

                // Prepare asesi data
                $asesiData = $this->prepareAsesiData($id, $userId);

                // Set signature in asesi data if available
                if ($newSignatureName) {
                    $asesiData['tanda_tangan'] = $newSignatureName;
                }

                // Update or insert asesi data
                if ($isUpdate) {
                    if (!$this->asesiModel->update($id, $asesiData)) {
                        throw new \Exception('Gagal mengupdate data asesi.');
                    }
                } else {
                    if (!$this->asesiModel->insert($asesiData)) {
                        throw new \Exception('Gagal menyimpan data asesi.');
                    }

                    // Additional logic for new entries (APL1, etc.)
                    // Add your additional inserts for related tables here
                }
            }
            ...
            return redirect()->to($isUpdate ? '/asesi' : '/dashboard');
        } catch (\Exception $e) {
