...
<script>
     ...
     * - Integration with DataTable for listing
     */
    const KelompokKerjaModule = (function() {
        ...
        // Configuration
        const config = {
            baseUrl: document.querySelector('base')?.href || '<?= base_url() ?>',
            selectors: {
                table: '#table-kelompok-kerja',
            ...
            endpoints: {
                getDataTable: '<?= base_url(); ?>/master/kelompok-kerja/get-data-table',
                getUnit: '<?= site_url('api/get-unit-json') ?>',
                getKelompokDetail: '<?= site_url('master/kelompok-kerja/detail') ?>',
                save: '<?= site_url('master/kelompok-kerja/save') ?>',
                delete: '<?= site_url('master/kelompok-kerja/delete') ?>'
            },
            messages: {
                ...
                confirmDelete: 'Data kelompok kerja ini akan dihapus secara permanen',
        ...
        function init() {
            ...
            // Attach event handlers for the list view
            attachListViewEvents();

            // Attach event handlers for the form
            attachFormEvents();
        ...
        function initDataTable() {
            ...
            // Add index and action columns (assuming DataTableHelper exists)
            if (typeof DataTableHelper !== 'undefined') {
                ...
                dataTable = DataTableHelper.initServerSideTable(
                    ...
                    config.endpoints.getDataTable,
                    columnsWithActions,
                    options
                );
            } else {
                console.warn('DataTableHelper is not defined. DataTable may not work properly.');
            }
        }
        ...
        function attachListViewEvents() {
            // Edit button click handler (for dynamically added elements)
            $(document).on('click', '.btn-edit', function() {
                const id = $(this).data('id');
                openModal('edit', id);
            });

            // Delete button click handler (for dynamically added elements)
            ...
            $(config.selectors.addButton).on('click', function() {
                openModal('add');
            });
        }
        ...
        function attachFormEvents() {
            ...
            $(config.selectors.modal).on('hide.bs.modal', handleModalClose);

            // Modal show event - reset form
            $(config.selectors.modal).on('show.bs.modal', function(e) {
                // No need to check for relatedTarget since we're using our own openModal function
                // Form will be reset and set up in openModal
            });
        ...
        /**
         * Confirm deletion of a kelompok kerja
         */
        function confirmDelete(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    deleteKelompokKerja(id);
                }
            });
        }

        /**
         * Delete kelompok kerja via AJAX
         */
        ...
        function resetForm() {
            ...
            // We don't reset unitCache here so the cache remains available
        }
        ...
        function renderKelompokForEdit(kelompokData) {
            ...
            // Reset cache for skema that will be edited
            ...
            kelompokData.forEach(function(kelompok) {
                ...
                // Save units that need to be selected
                if (kelompok.units && kelompok.units.length > 0) {
                    $(`#kelompok-${index}`).data('units', kelompok.units);
                }

                // Load units and set values after loading is complete
                const selectElement = $(`#kelompok-${index} .skema-select`);
                if (kelompok.id_skema) {
                    loadUnitsForSkema.call(selectElement[0], true); // true parameter for edit mode
                }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        showErrorInWrapper(wrapper, 'Gagal memuat unit kompetensi');
                    }
                },
                error: function(xhr, status, error) {
                    console.error("Error loading units:", error);
                    showErrorInWrapper(wrapper, config.messages.loadError);
                }
            });
        }

        /**
         * Show error message in a specific wrapper
         */
        function showErrorInWrapper(wrapper, message) {
            wrapper.html(`
            <div class="alert alert-danger">
                <i class="fas fa-exclamation-circle mr-1"></i> ${message}
            </div>
        ...
        /**
         * Set selected units after they've been loaded with a delay
         */
        function setSelectedUnitsAfterDelay(kelompokElement) {
            // Add a small delay to ensure select options are rendered
            setTimeout(() => {
